package com.varsha.branchandbound;

//Java Program to implement 0/1 knapsack using LC Branch and Bound

import java.util.*;

class Item {

	float weight; // Stores the weight of items
	int value; // Stores the values of items - profits
...
class Node {
	...
	float ub; // Upper Bound: Best case (Fractional Knapsack)
	float lb; // Lower Bound: Worst case (0/1)
	int level; // Level of the node in the decision tree
	boolean flag; // Stores if the current item is selected or not

	float tv; // Total Value: Stores the sum of the values of the items included
	float tw; // Total Weight: Stores the sum of the weights of included items
...
class sortByRatio implements Comparator<Item> {
	public int compare(Item a, Item b) {
		...
		return temp ? -1 : 1; //sorted in descending order
	}
}
...
public class O1KnapsackProblem {
	...
	public static void solve(Item arr[]) {
		...
		Arrays.sort(arr, new sortByRatio()); // Sort the items based on the profit/weight ratio, so best one is chosen 1st
		...
		pq.add(current); // Insert a dummy node
		...
		while (!pq.isEmpty()) {
			...
			if (current.ub > minLB || current.ub >= finalLB) {
				// if the current node's best case value is not optimal than minLB, then there is no reason to explore that node. Including
			...
			assign(right, 
					upperBound(current.tv, current.tw, level + 1, arr), 
			...
			} else { // If the left node cannot be inserted
